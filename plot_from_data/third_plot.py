...
for n in node_sizes:
    df = pd.read_excel(f"./../results/data_for_paper3/third/{file_mapping[n]}")
...
plt.plot([str(x) for x in node_sizes], stats['stretch_arrow_f256'], marker='v', linestyle='-.', label=f'Arrow', color=cmap(2), linewidth=1)
plt.plot([str(x) for x in node_sizes], stats['stretch_parrow_f256'], marker='^', linestyle=':', label=f'PArrow', color=cmap(4), linewidth=1)
plt.plot([str(x) for x in node_sizes], stats['stretch_f256'], marker='.', linestyle='-', label=f'OPArrow', color=cmap(0), linewidth=1)
